namespace Dimension19.AudioEvents.EditorWindows
{
    ...
    public class AudioEventsComponentEditorWindow : ExtendedEditorWindow
    {
        ...
        public float audioClipWaveform_Section_Height = 200;
        ...
        private Color audioWaveformBackgroundColor = Color.black;
        ...
        private Color audioWaveformForegroundColor = Color.yellow;
        ...
        private void OnEnable()
        {
            ...
            audioWaveformTexture2D.SetPixel(0, 0, AudioWaveform.background);
                ...
                GUILayout.BeginHorizontal("box", GUILayout.Height(top_Section_Height));
                {
                    ...
                    GUILayout.BeginVertical("box", GUILayout.Width(properties_Section_Width) , GUILayout.Height(top_Section_Height));
                    {
                        ...
                        //EditorGUI.indentLevel++;    // Make an indentation on the row
                        {
                            ...
                            audioClip = (AudioClip)EditorGUILayout.ObjectField(audioClipContent, audioClip, typeof(AudioClip), false); // The Audio Clip property
                        ...
                        // Background Color
                        audioWaveformBackgroundColor = EditorGUILayout.ColorField(audioWaveformBackgroundColorContent, AudioWaveform.background);
                        AudioWaveform.background = audioWaveformBackgroundColor;

                        // Foreground Color
                        audioWaveformForegroundColor = EditorGUILayout.ColorField(audioWaveformForegroundColorContent, AudioWaveform.foreground);
                        AudioWaveform.foreground = audioWaveformForegroundColor;
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            ...
                            //this.SendEvent(EditorGUIUtility.CommandEvent("DrawAudioWaveform")); // Draw the Audio Waveform on Startup

                            // Change the color of the Audio Event Marker Line
                            //audioEventsMarkerLine.SetPixel(0, 0, audioWaveformMarkerColor);
                            //audioEventsMarkerLine.Apply();
                        }
                    ...
                    GUILayout.BeginVertical("box", GUILayout.Height(top_Section_Height));
                    {
                        ...
                        Rect audioWaveform_Area = new Rect(properties_Section_Width + (GUI.skin.box.margin.horizontal * 2),
                                                           audioClipWaveform_Section_TopButtons_Height + (GUI.skin.box.margin.top * 4),
                                                           position.width - properties_Section_Width - (GUI.skin.box.margin.right * 7),
                                                           top_Section_Height - audioClipWaveform_Section_TopButtons_Height - (GUI.skin.box.margin.bottom * 4) - audioClipWaveform_Section_TopButtons_Height);
                            ...
                            audioWaveform_ScrollPos = EditorGUILayout.BeginScrollView(audioWaveform_ScrollPos);
                            {
                                if (audioClip != null)
                                {
                                    ...
                                    if (Event.current.commandName == "DrawAudioWaveform")
                                    {
                                        ...
                                        if (audioClip != null)
                                        {
                                            ...
                                            if (audioWaveform != null)
                                            {
                                                ...
                                                audioWaveformTextures = audioWaveform.GetWaveformAsTextures(((int)audioClip.length + 1) * audioClipWaveform_ZoomMagnifcationMultiplier_Max * 20, (int)audioWaveform_Area.height - 20, 0.7f);
                                            }
                                        }
                                    }
                                    ...
                                    if (new Rect(audioWaveform_Area.x, audioWaveform_Area.y, audioWaveform_Area.width, audioWaveform_Area.height - 20f).Contains(pos))
                                    {
                                        ...
                                        {
                                            audioWaveform_CursorHoverOverAtTime = Mathf.Clamp(e.mousePosition.x / audioClipWaveform_ZoomMagnificationMultiplier / 20, 0.0f, audioClip.length); // Update the time in the Audio Clip that the Cursor is currently hovered over
                                    ...
                                    {
                                        GUI.DrawTexture(new Rect(xPos, 0f, audioClip.length / audioWaveformTextures.Length * audioClipWaveform_ZoomMagnificationMultiplier * 20, audioWaveform_Area.height - 20f), audioWaveformTextures[i]); // Draw the current texture
                                    ...
                                    GUI.DrawTexture(new Rect(playheadPosition * audioClipWaveform_ZoomMagnifcationMultiplier_Min * audioClipWaveform_ZoomMagnificationMultiplier, 0f, 1f, audioWaveform_Area.height - 20f), audioPlayhead);

                                    // Draw Preview Audio Playhead for the Audio Clip being currently played
                                    if (isPlaying)
                                    {
                                        GUI.DrawTexture(new Rect(audioEventsComponent.audioSource.time * 20 * audioClipWaveform_ZoomMagnifcationMultiplier_Min * audioClipWaveform_ZoomMagnificationMultiplier, 0f, 1f, audioWaveform_Area.height - 20f), audioPreviewPlayhead);
                                    }
                        ...
                        //EditorGUI.indentLevel++; // Make an indentation on the row
                        {
                            ...
                            GUILayout.BeginHorizontal();
                            {
                                ...
                                if (GUILayout.Button(new GUIContent("Add at Time", "Adds an Audio Event at the Playhead's current time.")))
                                {
                                    ...
                                    if (arrayElement > 0)
                                    {
                                        audioEventsComponent.audioEvents[arrayElement] = new AudioEvent(audioEventsComponent.audioEvents[arrayElement - 1].onReached); // Create the new Audio Event element
                                    }
                                    else
                                    {
                                        audioEventsComponent.audioEvents[arrayElement] = new AudioEvent(); // Create the new Audio Event element
        ...
        private void DrawAudioEventsOnWaveform(Rect area)
        {
            ...
            for (int i = 0; i < audioEventsComponent.audioEvents.Length; i++)
            {
                ...
                GUI.DrawTexture(new Rect((float)audioEventsComponent.audioEvents[i].eventTime * audioClipWaveform_ZoomMagnifcationMultiplier_Min * audioClipWaveform_ZoomMagnificationMultiplier * 20, 0f, 1f, area.height), audioEventsMarkerLine);
                ...
                Rect audioEventButtonArea = new Rect((float)audioEventsComponent.audioEvents[i].eventTime * audioClipWaveform_ZoomMagnifcationMultiplier_Min * audioClipWaveform_ZoomMagnificationMultiplier * 20,
                ...
                if ((audioEventButtonArea.x + audioEventButtonArea.width) > (audioClip.length * audioClipWaveform_ZoomMagnifcationMultiplier_Min * audioClipWaveform_ZoomMagnificationMultiplier * 20))
                {
                    audioEventButtonArea = new Rect((float)audioEventsComponent.audioEvents[i].eventTime * audioClipWaveform_ZoomMagnifcationMultiplier_Min * audioClipWaveform_ZoomMagnificationMultiplier * 20 - audioEventMarkerSize.x + audioEventMarkerOffset.x,
                                                            audioClipWaveform_Section_TopButtons_Height + (GUI.skin.box.margin.top * 4) + audioEventMarkerOffset.y,
                                                            audioEventMarkerSize.x,
                                                            audioEventMarkerSize.y);
                }
